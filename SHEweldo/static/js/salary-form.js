...
document.getElementById("salaryForm").addEventListener("submit", async (e) => {
	...
	const formData = {
		...
		years_at_the_company: parseInt(
			document.getElementById("years_at_the_company").value
		),
		total_experience: parseInt(
		...
		is_well_compensated:
			document.getElementById("is_well_compensated").value === "true",
	...
	try {
		...
		if (response.ok) {
			...
			messageDiv.textContent =
				result.error || "Error submitting salary data";
		}
